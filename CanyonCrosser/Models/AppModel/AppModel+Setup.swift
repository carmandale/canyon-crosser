...
extension AppModel {
    ...
    // Create the elevation view that's attached to the hiker.
    func addHikerElevationView() {
        // Create the trailhead name attachment.
        let elevationAttachment = ViewAttachmentComponent(rootView: HikerElevationView())
        // Create the anchor to position and add it to the trailhead.
        let hikerElevationPositioningEntity = addBillboardingPositioningEntity(
            for: elevationAttachment,
    ...
    func removeHikerElevationView() {
        hikerEntity.childAt(path: hikerEntity.name + ".positioningEntity")?.removeFromParent()
    }
